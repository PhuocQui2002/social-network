...
function ListFollowing() {
  ...
  const unFollowingHander = async (idUser) => {
    //console.log(user?._id);
    try {
      const res = await UserService.followOrUnfollow(idUser);
      if (res.success) {
        const updatedFollowing = user.following.filter((id) => id !== idUser);
        dispatch(setAuthUser({ ...user, following: updatedFollowing }));
        toast.success("Unfollow thành công!");
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="my-6">
      <div className="flex items-center justify-between text-sm mb-2">
        <h2 className="font-semibold text-gray-600">Friends you follow</h2>
      ...
      <div className="max-h-[280px] overflow-y-auto pr-2 space-y-4 no-scrollbar">
        {friendsYouFollow?.length > 0 ? (
          friendsYouFollow?.map((user) => (
            <div key={user._id} className="flex items-center justify-between">
              ...
              <span
                ...
                onClick={() => unFollowingHander(user._id)}
              >
                Unfollow
              </span>
            </div>
